...
export namespace HawkeyeSearch {
  ...
  export async function searchMembers(library: string, sourceFile: string, memberFilter: string, searchTerm: string, readOnly?: boolean): Promise<SourceFileMatch[]> {
    ...
    if (connection) {
      ...
      cmdResult = await Code4i.runCommand({ command: runDSPSCNSRC, environment: `ile` });
  ...
  export async function displayFileSetsUsed(library: string, dbFile: string, searchTerm: string, readOnly?: boolean, resultSequence?: string): Promise<SourceFileMatch[]> {
    // Function steps.
  ...
  export async function displayFileKeys(library: string, dbFile: string, searchTerm: string, readOnly?: boolean, resultSequence?: string): Promise<SourceFileMatch[]> {
    ...
    if (connection) {
      ...
      const resultSetQty = await Code4i!.runSQL(`select count(*) as RS_QTY from ${tempLibrary}.${tempName1}`);
      if (resultSetQty.length === 0 || resultSetQty[0].RS_QTY === 0) { throw new Error(`No records found in Hawkeye database.`); }
      let keyListCommand = `with T1 as (
        select trim(TFDFIL) REF_FILE ,trim(TFDKEY) REF_KEYS ,trim(TFDTXT) REF_TEXT 
        ,case when APISTS='1' then APISF when TFDSFL=' ' then TFHSFL else TFDSFL end xTFDSFL
            ...
            , SEARCHMATCHES(SEARCHMATCH) as (select 
            json_object(
                'fileName': REF_FILE 
                ,'fileText': ifnull(min(trim(REF_TEXT)),'')
                ,'filePath' : '/QSYS.LIB/'|| trim(xTFDSLB) || '.LIB/' || trim(xTFDSFL) || '.FILE/' || trim(xTFDSMB) || '.' 
                    || (case when SQL_OBJECT_TYPE is null then OBJATTRIBUTE when SQL_OBJECT_TYPE is not null then 'SQL' else 'MBR' end)
                , 'matches' : json_arrayagg(json_object( 'lineNumber' : 1, 'content' : rtrim(REF_KEYS)) order by xTFDSLB, xTFDSFL, xTFDSMB, 1) 
                returning clob format json)    
            from T1
            group by xTFDSLB, xTFDSFL, xTFDSMB, OBJATTRIBUTE, SQL_OBJECT_TYPE, REF_FILE
          )
          select cast(SEARCHMATCH as varchar(32000)) SEARCHMATCH from SEARCHMATCHES order by SEARCHMATCH
          `.replace(/\n\s*/g, ' ');
      let keySourceScanResults = await Code4i.runSQL(keyListCommand, { forceSafe: true });
      if (keySourceScanResults && keySourceScanResults.length > 0) {
        ...
        //             select cast( SEARCHMATCH as varchar(32000)) SEARCHMATCH from SEARCHMATCHES order by SEARCHMATCH`.replace(/\n\s*/g, ' ');
        // let queryResults = await Code4i.runSQL(statement, { forceSafe: true });
        let queryResults = keySourceScanResults;
        const parsedRows = queryResults.map(row => parseSearchMatch(row.SEARCHMATCH));
        searchMatches = parsedRows
          .filter(row => row && Object.keys(row).length > 0) // filter out empty objects
          .map(row => ({
            ...
            matchCount: Array.isArray(row.matches) ? row.matches.length : 0,
            matches: Array.isArray(row.matches) ? row.matches : [],
            searchTokens: [`${connection.sysNameInAmerican(library)}/${connection.sysNameInAmerican(dbFile)}`.toLocaleUpperCase()]
          } as SourceFileMatch));
      } else {
        throw new Error(l10n.t('No results for Display File Set Used.'));
      }
  ...
  export async function displayProgramObjects(library: string, program: string, type: string, searchTerm: string, readOnly?: boolean): Promise<SourceFileMatch[]> {
    ...
    if (connection) {
      ...
      let cmdResult = await Code4i.runCommand({ command: runDSPPGMOBJ, environment: `ile` });
      if (cmdResult.code !== 0) { throw new Error(`${connection.sysNameInAmerican(library)}/${connection.sysNameInAmerican(program)}    \n` + cmdResult.stderr); }
      const resultSetQty = await Code4i!.runSQL(`select count(*) as RS_QTY from ${tempLibrary}.${tempName1}`);
      if (resultSetQty.length === 0 || resultSetQty[0].RS_QTY === 0) { throw new Error(`No records found in Hawkeye database.`); }
      // There are three DSPSCNSRC commands.  One to produce a search hit for the program objects of the initial program object, so you can open the source and the other for the search hit of the program object within the initial program object.  The third DSPSCNSRC is used to match the extra search token within the inital program object.  Thgis third is separate to reduce the amount of results output to the work table.
      ...
      select qcmdexc('DSPSCNSRC SRCFILE('||trim(PODSLB)||'/'||trim(PODSFL)||') SRCMBR('||trim(PODSMB)||') TYPE(*ALL) OUTPUT(*OUTFILE) OUTFILE(${tempLibrary}/${tempName2}) OUTMBR(HWKDSPPO *ADD) SCAN( '''||trim(PODOBJ)||''') LOGIC(*AND) CASE(*IGNORE) BEGPOS(001) ENDPOS(240)') CA
      , qcmdexc('DSPSCNSRC SRCFILE(*SRCL/Q*) SRCMBR(${program}) TYPE(*ALL) OUTPUT(*OUTFILE) OUTFILE(${tempLibrary}/${tempName2}) OUTMBR(HWKDSPPO *ADD) SCAN('''||trim(PODOBJ)||''') CASE(*IGNORE) BEGPOS(001) ENDPOS(240)') CB from T1 where PODSMB <> '${program}' union select * from table( values ( qcmdexc('DSPSCNSRC SRCFILE(*SRCL/Q*) SRCMBR(${program}) TYPE(*ALL) OUTPUT(*OUTFILE) OUTFILE(${tempLibrary}/${tempName2}) OUTMBR(HWKDSPPO *ADD) SCAN(${sanitizeSearchTerm(searchTerm) ? `''${sanitizeSearchTerm(searchTerm)}''  ` : ""}) CASE(*IGNORE) BEGPOS(001) ENDPOS(240)') ,QCMDEXC('DUMMY_CMD'))) THIRD (CA, CB)`.replace(/\n\s*/g, ' '));
      if (dpoSourceScanResults && dpoSourceScanResults.length > 0) {
        ...
        const parsedRows = queryResults.map(row => parseSearchMatch(row.SEARCHMATCH));
        let searchTokens: any[] = [];

        searchMatches = parsedRows
          .filter(row => row && Object.keys(row).length > 0) // filter out empty objects
          .map((row) => {
            if (row.fileName !== program) {
              // capture all  object tokens for later use
              searchTokens.push(row.fileName);
            }
            return {
              fileName: row.fileName,
              filePath: row.filePath,
              ...
              matchCount: Array.isArray(row.matches) ? row.matches.length : 0,
              matches: Array.isArray(row.matches) ? row.matches : [],
              searchTokens: [row.fileName]
            } as SourceFileMatch;
          }
          );
        // store captured object tokens for later use
  ...
  export async function displayObjectUsed(library: string, object: string, objType: string, searchTerm: string, howUsed: string, readOnly?: boolean): Promise<SourceFileMatch[]> {
    const connection = Code4i.getConnection();
    ...
    if (connection) {
      ...
      let cmdResult = await Code4i.runCommand({ command: runDSPOBJU, environment: `ile` });
  ...
  export async function displayProcedureUsed(procedure: string, searchTerm: string, readOnly?: boolean): Promise<SourceFileMatch[]> {
    ...
    if (connection) {
      ...
      let cmdResult = await Code4i.runCommand({ command: runDSPPRCU, environment: `ile` });
